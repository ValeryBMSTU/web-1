...
4. Черзе интерфейс GitHub cделайте fork данного репозитория, после чего склонируйте его на свою локальную машину.
5. Создайте локально от ветки master/main ветку dev. Сделайте отчет в формате doc(x)/pdf и добавьте его в ветку dev, после чего зафиксируйте изменения с помощью команды commit
6. Запушьте локальную dev-ветку в удаленный репозиторий GitHub и создайте merge request из dev в master/main
7. Продемонстрируйте результаты в ходе защиты лабораторной работы
